...
<table class="table table-striped" width="100%" border="1px">
			...
			while ($row=$dt->fetch()) {
				...
				<tr>
					<td><?php echo $stt; ?></td>
					<td><a href="index.php?view=feedback&action=edit&id=<?php echo $row['id'] ?>" class="navbar-link"><?php echo $row['name'] ?></a></td>
		...
		<div class="row">
			<ul class="pagination pagination-sm m-0 float-right">
				...
				if (isset($_GET['pag']) && $_GET['pag'] > 1) {
					$prev=$_GET['pag']-1;
					echo "<li  class='page-item'><a class='page-link' href='index.php?view=feedback&pag=".$prev."'>&laquo;</a></li>";
				}
				?>

				<?php
					$dt->select("SELECT* FROM contact_form");
					$count=$dt->num_rows();
					...
					for ($i=1; $i <=$total_pag ; $i++) { 
						...
						echo "<li class='page-item ".$active."'><a class='page-link' href='index.php?view=feedback&pag=$i'> $i </a></li>";
