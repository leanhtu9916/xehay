...
<?php
	...
	if ($id !="") {
		...
		$dt->select("SELECT * FROM product,type,fuel,engine WHERE product.type = type.id AND  product.fuel = fuel.id AND  product.engine = engine.id AND category='$category' AND status='Xe mới' ORDER BY product_id desc LIMIT $pag,12");
	}else {
		$dt->select("SELECT * FROM product,type,fuel,engine WHERE product.type = type.id AND  product.fuel = fuel.id AND  product.engine = engine.id AND status='Xe mới' ORDER BY product_id desc LIMIT $pag,12");
	}
?>
		...
		<div class="singular-post-content list-item-car">
			...
			<?php
					if ($id !="") {
						...
						$dt->select("SELECT * FROM product,type,fuel,engine WHERE product.type = type.id AND  product.fuel = fuel.id AND  product.engine = engine.id AND status='Xe mới' ");
						$count=$dt->num_rows();
					}
		            $pag2=ceil($count/12);
